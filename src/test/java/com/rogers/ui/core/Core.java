...
public class Core {
	...
	public Core() throws Exception {
		if (getLocalDriver() == null) {
			...
			if (getEXECUTION_MACHINE().toLowerCase().equals("local")) setForLocal(); 
			else setForGrid();
...
//		driver = new ChromeDriver(capabilities);
	...
	private void setForFirefox() throws MalformedURLException {	
		WebDriverManager.firefoxdriver().version("0.20.0").setup();
		capabilities = DesiredCapabilities.firefox();
//		capabilities.setCapability("marionette", false);
//		driver = new FirefoxDriver(capabilities);
		setLocalDriver(new FirefoxDriver(capabilities));
	}
